...
- [ ] Fix 3 — Timestamp truncation: `onUiMidiEvent` stores `tv_nsec` only, not full monotonic ns
- [ ] Fix 4 — Wrong channel: `onUiMidiEvent` hardcodes `channel=0`; drum hits replay through piano
- [ ] Fix 5 — `playedIndex` never updated when all events in window fire — events replay forever
- [ ] Fix 6 — `AudioGraph.h` still declares `loopRecordEvent` whose body was deleted from `.cpp`
- [ ] Fix 7 — `current` member has no in-class initializer — UB on first `advance()` call
- [ ] Fix 8 — `clear()` does not reset `playedIndex` or `current` — first pass after re-record is silent
